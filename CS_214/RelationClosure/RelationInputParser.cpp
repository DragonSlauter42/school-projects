...
// Default constructor
//------------------------------------------
RelationInputParser::RelationInputParser()
{
}
...
//------------------------------------------
RelationInputParser *RelationInputParser::getInstance()
{
	static RelationInputParser *theInstance = NULL;
	if(theInstance == NULL)
	{
		theInstance = new RelationInputParser();
	}
	return theInstance;
}
...
//------------------------------------------
void RelationInputParser::initDataParser(char *filename)
{
	char line[128];
    int i=0; //Counter for the element 
	
//	strcpy_s(m_sFileName, strlen(filename) + 1, filename); 
    memcpy(m_sFileName, filename, strlen(filename) + 1); 
	inFile = new ifstream();
...
int RelationInputParser::getOrder(string X)
{
	for (int i = 0; i < 10; i++)
...
//-------------------------------------------------------------------
bool RelationInputParser::getRelationElement(int X, int Y)
{	
	return m_bElements[X][Y]; // 
}

int RelationInputParser::getRelationSize()
{
	return m_iSetSize;
}


string RelationInputParser::getElement(int X)
{
...
//------------------------------------------------
bool RelationInputParser::getNextLine(char *buffer, int n)
{
	bool    done = false;
	char    tempBuf[128];
	char	*temp;
